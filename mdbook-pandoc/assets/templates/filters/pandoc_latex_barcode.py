...
def barcode(doc, data, opt, size):
    barcode = (
        doc.barcode_begin + size + "\\psbarcode{" + data + "}" + opt + doc.barcode_end
    )
    return [latex(barcode)]


def prepare(doc):
    doc.barcode_barcode_width = doc.get_metadata(
        "barcode.barcode_width", default="50mm"
    )
    doc.barcode_barcode_height = doc.get_metadata(
        "barcode.barcode_height", default="30mm"
    )
    doc.barcode_qrcode_width = doc.get_metadata("barcode.qrcode_width", default="25mm")
    doc.barcode_qrcode_height = doc.get_metadata(
        "barcode.qrcode_height", default="25mm"
    )
...
def action(elem, doc):
    if isinstance(elem, pf.Div) and doc.format == "latex":
        if BARCODE in elem.classes:
            opt = "{includetext}"
            size = (
                "(" + doc.barcode_barcode_width + "," + doc.barcode_barcode_height + ")"
            )
        ...
        if QRCODE in elem.classes:
            ...
            opt += "{qrcode}"
            size = (
                "(" + doc.barcode_qrcode_width + "," + doc.barcode_qrcode_height + ")"
            )
            data = pf.stringify(elem)
            return barcode(doc, data, opt, size)
